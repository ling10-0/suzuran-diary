...
function MunicipalHome(){
 ...
 return <div className="municipal-site">
  ...
  <div className="office-shell">
   <aside className="office-sidebar"><section><h2>公示板目錄</h2><button onClick={()=>scroll('new-notices')}>一、本日新到公告</button><button onClick={()=>scroll('patrol')}>二、兩日巡回路線</button><button onClick={()=>scroll('roster')}>三、職員編制表</button><button onClick={()=>scroll('office-map')}>四、市街配置圖</button></section><section className="office-counter"><h2>本日洽公人數</h2><strong>{visits}</strong><p>本月累計　〇九一三件</p></section><section className="office-small-notice"><h2>洽詢須知</h2><p>巡查者須攜帶調查簿。案件答案不明者，請於現地重新核對，不得逕向文書係索取。</p></section></aside>
   <main className="office-main">
    <section className="office-welcome" id="new-notices"><div className="office-title-row"><p>臺中市役所告示</p><h2>本日新到公告</h2><span>揭示期間：昭和十三年八月</span></div><div className="notice-list"><article><time>八月十三日</time><div><small>庶務秘第七二號</small><h3>臨時調查員任命及市街巡查要領</h3><p>因市役所舊簿冊出現缺頁，命受理學生編為見習調查員，依指定區域採集民生日常及商號紀錄。</p></div><button onClick={()=>open('./?page=guide')}>公告本文</button><i>受理</i></article><article><time>八月十三日</time><div><small>巡查第十九號</small><h3>兩日巡回路線及配置變更</h3><p>本次巡查分兩日共十七處，應依公告順序完成查錄；各點詳見配置圖及附屬路線頁。</p></div><button onClick={()=>scroll('patrol')}>路線公告</button><i>已閱</i></article><article><time>八月十四日</time><div><small>文書秘第二號</small><h3>十三件調查案件開放查核</h3><p>現地所得答案可於案件目錄逐件查核。內容相符者，准予閱覽附屬公告及未綴込文書。</p></div><button onClick={()=>open('./?page=puzzles')}>案件目錄</button><i>秘</i></article></div></section>
   ...
   <aside className="office-rightbar"><section><h2>執務狀態</h2><p><b>受付</b><span>午前九時開始</span></p><p><b>巡查</b><span>兩日十七處</span></p><p><b>案件</b><span>十三件待查</span></p><p><b>終章</b><span>第四市場</span></p></section><section className="office-counter compact"><h2>本日登入人數</h2><strong>{visits}</strong><p>本機到訪次數</p></section><section className="office-clipping"><h2>係員備忘</h2><p>「正式紀錄」與「人們記得的事」若有出入，兩者均須留下。</p><span>— 文書係頁緣註記</span></section><section><h2>案件受付</h2><p className="rightbar-explain">取得現場答案後，前往案件目錄逐件查核。</p><button onClick={()=>open('./?page=puzzles')}>開啟十三件調查案件</button></section></aside>
  </div>
